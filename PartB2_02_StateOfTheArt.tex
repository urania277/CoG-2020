...
\\
\indent 
Exploiting this \textbf{synergy between different experiments} requires a well-specified \textbf{common theoretical basis}. 
...
\indent
While additional interactions and modifications are required to make simplified models self-consistent (see e.g.~\cite{Ellis:2018xal}), they do not change the main experimental feature: given that the mediator has been produced from the interaction of SM particles, it can also decay into SM particles. 
...
A \textbf{new U(1) symmetry} introduces a portal particle called the \textit{dark photon}~\cite{Holdom:1985ag,Curtin:2014cca}.   
The dark photon mixes with the SM photon, leading to rare but observable signatures from its decays to SM particles. 
...
Innovation within the ATLAS data acquisition and selection system is an integral part of this proposal, 
...
These limitations impair the sensitivity of searches where signals are buried in high-rate backgrounds and motivates the improvements in WP1. 
...
In the processes sought in traditional searches via a mediator particle as described in Ref.~\ref{sub:stateOfTheArtTheory}, 
...
In a similar vein to the Data Scouting technique in CMS~\cite{ToBeCited} and the Turbo Stream in LHCb~\cite{ToBeCited}, my StG team and I prototyped the \textbf{Trigger Level Analysis (TLA) technique} at ATLAS for jets. 
...
\\
\indent
In normal data-taking conditions, TLA dijet searches are still limited by the L1 event rate to resonances with masses above 450 GeV, 
...
This is a region where the weak force mediators reside, and that is favored in WIMP and non-WIMP $Z’$ models that are consistent with the measured ID excesses~\cite{ToBeCited}. %Hooper&Leane  
...
the state-of-the-art in \textbf{hidden sector and dark QCD searches} leaves much room for joint theoretical and experimental improvement.

Hidden sector models generally imply more difficult detector signatures than the established benchmarks. 
...
From this classification, \textbf{Semi-visible jets}  (Fig.~\ref{fig:feynman} [c]) are not covered experimentally at the time of writing. 
%Should I say this? 
...
Experimental advances generally stimulate theoretical development and extensions of unprobed models, as well as efforts towards their systematic classification. 
As part of WP5 in this proposal, I will organize workshops and open a number of discussion channels with the theory, astroparticle, non-collider and nuclear physics communities. 
